...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        //tableView init
        _leftTableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x, self.frame.origin.y + self.frame.size.height, UI_SCREEN_W*0.5, 0) style:UITableViewStylePlain];
...
//        _rightTableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x+UI_SCREEN_W*0.3, self.frame.origin.y + self.frame.size.height, UI_SCREEN_W*0.7, 0) style:UITableViewStylePlain];
...
- (void)animateBackGroundView:(UIView *)view show:(BOOL)show complete:(void(^)())complete {
    if (show) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            view.backgroundColor = [UIColor colorWithWhite:0.9 alpha:0.4];
        }];
    } else {
        [UIView animateWithDuration:0.2 animations:^{
    ...
    if (self.delegate || [self.delegate respondsToSelector:@selector(menu:tableView:didSelectRowAtIndexPath:)]) {
        if (tableView == self.leftTableView) {
            [self animateBackGroundView:_backGroundView show:YES complete:^{
                [self animateTableViewShow:YES complete:^{
                    _show = YES;
                }];
            }];
        }
